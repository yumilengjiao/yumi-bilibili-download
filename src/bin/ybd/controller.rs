...
async fn download_video(
        ...
        if batch {
                ...
                let mut pn = 1usize;
                loop {
                        let resp = get_basic_collection_info(
                                &ml_id,
                                pn,
                                concurrencies,
                                Some(&bili_client),
                ...
                for bv_id in bv_ids {
                        ...
                        let jh = tokio::spawn(async move {
                                ...
                                let video_path = base_path.join(format!(
                                        "{}.mp4",
                                ...
                                let dp_video =
                                        DownloadProgress::new(format!("{} [视频]", bv_id), 0);
                                let dp_audio =
                                        DownloadProgress::new(format!("{} [音频]", bv_id), 0);
...
async fn download_cover(
        ...
        if batch {
                ...
                for btc in bv_title_covers {
                        ...
                        let jh = tokio::spawn(async move {
                                let _permit = sp.acquire().await?;
                                let cover_path = base_path.join(format!(
                                        "{}.png",
                                        sanitize_filename::sanitize(&btc.1)
                                ));
